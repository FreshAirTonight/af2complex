...
The output structural models are under the subdirectory `af2c_mod`. By default, this run uses the `monomer_ptm` models. You may also test the `multimer_np` mode, which assembles input features using unpaired MSAs and runs the `multimer_x_multimer` models for inference. For the purpose of comparison,
...
./run_relaxation.sh
```
This run will run the relaxation protocol on all un-relaxed structural models generated by running `run_af2comp.sh`. Note that the relaxation is molecular dynamics minimization, which cannot eliminate severe clashes as observed in some models of large complexes generated with the multimer DL models.
