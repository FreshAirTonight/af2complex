...
#
# Predicting models of a protein complex using either monomer or multimer DL models
# Supports both paired MSAs (by AF-Multimer) and unpaired MSAs
# Metrics designed for evaluating multimeric protein interactions
# Option to save structure models of intemediate recycles
# Checkpoint for model inference
# Domain cropping
# Staged workflow: feature generation (af2c_fea), DL inference (af2c_mod, this script), and model relaxation (af2c_min)
#
...
flags.DEFINE_enum('preset', None,
                  ['reduced_dbs', 'casp14', 'economy', 'super', 'expert', 'super2', 'genome', 'genome2'],
...
flags.DEFINE_integer('msa_crop_size_mono', 5000, 'For additional monomer MSA cropping in multimer_np mode', lower_bound=1)
...
##################################################################################################
def _process_target_name( target_name ):
...
def main(argv):
  ...
  for model_name in FLAGS.model_names:
    ...
    if 'monomer' in FLAGS.model_preset and 'multimer' in model_name:
        raise ValueError(f"{model_name} is not compatible with {FLAGS.model_preset}" )
    elif 'multimer' in FLAGS.model_preset and not 'multimer' in model_name:
        raise ValueError(f"{model_name} is not compatible with {FLAGS.model_preset}" )

    model_config = config.model_config(model_name)
    if 'multimer' not in FLAGS.model_preset:
